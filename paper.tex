...
\end{equation}
where \keith{F) $p_i$ is the probability mass function of the walker, $n=2\,t_\mathrm{max}+1$ is the number of lattice sites, and $\mu=\sum_{i=1}^{n}{p_{i} i}$ is the mean of the distribution. For calculating the variance in two-dimensions we convert the probability distribution to a one-dimensional probability distribution by setting the $i$th element to be the sum over the columns of the two-dimensional distribution.} Fig.~\ref{fig:PescVsTime} (top) illustrates the variance versus time for both a QRW and a CRW on a two-dimensional square lattice of size $t_\mathrm{max}=20$. The QRW demonstrates a quadratic rate of spreading across the lattice while the CRW demonstrates a linear rate of spreading. This quadratic spreading is one of the distinguishing features of a QRW compared to the CRW. It forms the basis of some QRW algorithms such as the QRW search algorithm, which is quadratically faster than the corresponding classical algorithm. \keith{D,E) For simulations of the variance the walker never reaches the boundaries because it begins at the centre of the square \mbox{$(2\ t_{\mathrm{max}}+1)^2$} lattice and evolves for $t_{\mathrm{max}}$ time steps. It is as though there are no boundaries in the variance simulations and thus no boundary conditions are applied.} 
...
\keith{D,E) For all escape probability simulations the walker is initialized at $\ket{\Psi(0)}=\ket{-t_{\mathrm{max}},0,1,1}$; therefore, we impose reflecting boundary conditions. We impose reflecting boundaries with the bitflip coin,
...
The two metrics, $\sigma^{2}$ and $P_{\mathrm{esc}}$, are closely related. If the walker has a large spread in its distribution then the walker also has a better chance to fall outside of the escape boundary but they also capture different aspects of the distribution. At any given time step $t$ during the evolution we can determine the probability distribution over the lattice with Eq.~\ref{eq:probX} and then calculate these various metrics to be used for quantifying a random walk. \keith{A) Any non-deterministic distribution obtained in this paper was obtained using a Monte-Carlo averaging technique. Since the sample space we are averaging over grows exponentially we are limited to lattice sizes of about $t_{\mathrm{max}}=\{1,20\}$.} \keith{B) Many transport phenomena that is studied requires system sizes on the order of $10^4$ to observe certain unique behaviour. Although we do not observe these phenomena due to our small lattice sizes we make interesting observations about quantum walkers on congested lattices with and without dephasing.} Next, we demonstrate how to add spatial defects, which cause congestion, into the walkers' lattice and explore how the variance and escape probability are affected by this lattice congestion. 
...
\caption{\keith{H) The variance $\sigma^2$ and the escape probability $P_{\mathrm{esc}}$ for a QRW plotted as a function of time $t$ for varying congestion probabilities $1-p$ on a two-dimensional square lattice of size $t_\mathrm{max}=15$. (Top) The walker is initialised at $\ket{\Psi(0)}=\ket{0,0,1,1}$. Reduced spreading is observed as congestion increases. The CRW is also shown for $p=0.7$ which illustrates that the QRW maintains it's advantages with congestion. (Bottom) The escape boundary is at $t_\mathrm{b}=4$ and input state $\ket{\Psi(0)}=\ket{-t_\mathrm{max},0,1,1}$. As $p$ decreases the jump in $P_{\mathrm{esc}}$ becomes less prominent. The CRW is presented for $p=0.7$ to illustrate the reduced rate of escape compared to a QRW with the same value of congestion.}} \label{fig:PescVsTVaryingP}
\end{figure}
%***********************************

%varying t_b
\keith{\subsection{Varying escape boundary}
C, K) In the previous simulations involving escape probability the escape boundary was set to be near the ititialized position of the walker. The next thing we consider on a congested lattice is how the escape probability on a congested lattice changes as $t_\mathrm{b}$ varies. Consider Fig. \ref{fig:VarTb} which shows $P_{\mathrm{esc}}$ as a function of $t_\mathrm{b}$ with varying values of congestion $p$ for the CRW (Top) and the QRW (Bottom). Both walkers began in the state $\ket{\Psi(0)}=\ket{-t_{\mathrm{max}},0,1,1}$ and evolved for $t_{\mathrm{max}}=10$. $P_{\mathrm{esc}}$ in this case was calculated at $t=t_{\mathrm{max}}$ and the statistics were averaged over many numerical simulations. In both the CRW and QRW $P_{\mathrm{esc}}$ reduces with increased congestion and when $t_\mathrm{b}$ moves farther from the walkers initial position. The QRW maintains a much larger $P_{\mathrm{esc}}$ as the boundary moves than the CRW. What is interesting is that for both the QRW and CRW $P_{esc}=0$ on average for $t_b\gtrsim 8$ using these initial conditions. 

The simulations of $P_{\mathrm{esc}}$ done in this manuscript are in the transient regime, whereby the longterm dynamics have not set in. Although our simulations are limited to small lattice sizes of about $t_{\mathrm{max}}=\{1,20\}$ or so, Figs. \ref{fig:VarTb} helps to observe what happens outside of the transient regime. We see that when the walker approaches the end of it's evolution (i.e. $t\approx t_{\mathrm{max}}$) that $P_{\mathrm{esc}}$ approaches zero. Therefore, we also expect $P_{\mathrm{esc}}$ in the non-transient regime to approach zero near the end of the lattice.

(my argument here about transient regime is not very strong I believe. How can we make this stronger?

also, I lost the data for the top plot in this section. I am rerunning it so that we can move the p values to the right)}

%**********Figure*****************
\begin{figure}[] 
\includegraphics[scale=0.508]{PEscVarTbClass.pdf}
...
%***********************************
 ...
 With sufficiently strong dephasing the probability distribution becomes strongly localized around the origin so the QRW becomes identical to a CRW distribution. \keith{O) This localization is related to Anderson localization \cite{bib:PhysRev.109.1492}.} This is shown in Fig.~\ref{fig:ThreeDBinomial} for the same input state and lattice size as in Fig.~\ref{fig:AndersonNoDephasing} but with $p_{\mathrm{d}}=0.0005$.
%**********Figure*****************
\begin{figure}[]
...
\end{figure}
%***********************************
This value of $p_\mathrm{d}$ corresponds to just a few sign errors on average during the walk which can cause the whole QRW to behave classically. By incrementing the dephasing through this interval we can smoothly interpolate between QRWs and CRWs, which is a key feature of this work. \keith{M) Note that the dephasing probability $p_\mathrm{d}$ depends on $t_\mathrm{max}$. As the lattice size grows the dephasing required to obtain the classical distribution is smaller due to there being more total sites where dephasing can occur.}
